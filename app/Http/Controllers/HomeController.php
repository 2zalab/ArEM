...
class HomeController extends Controller
{
    ...
    {
        $statistics = [
            'total_documents' => Document::where('status', 'published')->where('is_hidden', false)->count(),
            'total_authors' => User::where('role', 'depositor')->count(),
            'total_departments' => Department::where('is_active', true)->count(),
            'total_downloads' => Document::where('status', 'published')->where('is_hidden', false)->get()->sum(fn($doc) => $doc->getTotalDownloads()),
        ];
